...
$query = "SELECT id FROM third_party_user_accounts WHERE user_id = '$userId'";
...
if (mysqli_num_rows($result) > 0) {
    // Update the existing user's nickname for the given game
    $row = mysqli_fetch_assoc($result);
    $accountId = $row['id'];

    $query = "UPDATE third_party_user_accounts SET username = '$newNickname', game_id = '$gameId' WHERE id = '$accountId'";
} else {
    // Insert a new nickname for the game and user
